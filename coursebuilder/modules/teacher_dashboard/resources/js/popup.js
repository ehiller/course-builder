...
SectionTable.prototype = {
  _buildRow: function(section) {
    ...
    var td = $(
        '<td class="roster">' +
        '  <a class="gcb-button" role="button" href="/sample/modules/teacher_dashboard?action=teacher_dashboard' +
 ...
 */
function AddStudentsPopup(sectionId, xsrfToken) {
  var that = this;
  this._sectionId = sectionId;
  ...
  this._form.find('button.add-students-cancel-button')
    .click(function() {
      that._onCancel();
      return false;
    }
...
AddStudentsPopup.prototype = {
   ...
   */
  open: function(callback) {
    this._onAjaxAddStudentCallback = callback;
    this._lightbox
      .bindTo(this._documentBody)
      .setContent(this._form)
      .show();
  ...
  _onSave: function() {
    ...
    function onUpdatedStudents(student_list, message) {
      showMsgAutoHide(message);
      that._onAjaxAddStudentCallback(student_list);
    }
    this.updateStudents(onUpdatedStudents, emails, that._sectionId, that._xsrfToken);
    this._lightbox.close();
  },
   ...
   * @param sectionId
   */
  updateStudents: function(callback, emails, sectionId, xsrfToken) {
