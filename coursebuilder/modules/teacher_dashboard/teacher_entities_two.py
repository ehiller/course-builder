...
class Teacher(Student):
    ...
    @property
    def students(self):
        return self.key().students()  # not sure about this method
    ...
    def add_new_teacher_for_current_user(
        ...
        TeacherProfileDAO.add_new_student_for_current_user(
    ...
    @classmethod
    def _get_user_and_teacher(cls):
        """Loads user and teacher and asserts both are present."""
    ...
    def rename_current(cls, new_name):
        """Gives teacher a new name."""
        _, teacher = cls._get_user_and_teacher()
        TeacherProfileDAO.update(
            teacher.user_id, teacher.email, nick_name=new_name)

    @classmethod
    def set_enrollment_status_for_current(cls, is_enrolled):
        """Changes teacher enrollment status."""
        _, teacher = cls._get_user_and_teacher()
        TeacherProfileDAO.update(
            teacher.user_id, teacher.email, is_enrolled=is_enrolled)
...
class TeacherProfileDAO(object):
    ...
    @classmethod
    def _get_profile_by_user_id(cls, user_id, email=None):
        """Loads profile given a user_id and returns Entity object."""
        old_namespace = namespace_manager.get_namespace()
        try:
            namespace_manager.set_namespace(cls.TARGET_NAMESPACE)

            profile = MemcacheManager.get(cls._memcache_key(user_id), namespace=cls.TARGET_NAMESPACE)
            if profile == NO_OBJECT:
                return None
            if profile:
                return profile
            profile = PersonalProfile.get_by_key_name(user_id)
            MemcacheManager.set(cls._memcache_key(user_id), profile if profile else NO_OBJECT,
    ...
    def _update_global_profile_attributes(
            ...
            course_info=None):
        """Modifies various attributes of Teacher's Global Profile."""
        ...
        if not (is_enrolled is None and final_grade is None and
            ...
            if course_info is not None or final_grade is not None:
                course_info_dict = {}
    ...
    def _update_course_profile_attributes(
        ...
        """Modifies various attributes of Teacher's Course Profile."""
    ...
    def get_enrolled_teacher_by_email_for(cls, email, app_context):
        ...
        try:
            ...
            return Teacher.get_enrolled_teacher_by_email(email)
        finally:
            namespace_manager.set_namespace(old_namespace)
    ...
    def update(
        ...
        profile, student = cls._update_in_txn(
            user_id, email, legal_name, nick_name, date_of_birth, is_enrolled, final_grade,
            course_info, labels, profile_only)
        common_utils.run_hooks(
            cls.UPDATE_POST_HOOKS, profile, student, user_id, email,
            legal_name, nick_name, date_of_birth, is_enrolled, final_grade,
            course_info, labels, profile_only)
    ...
    def _update_in_txn(
        ...
        if not profile_only:
            student = Student.get_by_email(email)
            if not student:
                raise Exception('Unable to find teacher for: %s' % user_id)
